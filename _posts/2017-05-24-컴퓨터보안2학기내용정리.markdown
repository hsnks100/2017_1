...
작업 순서  
1. A의 개인키 $$A_x1$$을 생성하고  
$$y1 = g^x1\;mod\;p$$ 으로 공개키 y1 를 계산한다.  
2. B에게 y1 를 전달한다.  
3. B는 개인키 $$B_x2$$를 생성하고 같은 방식으로 공개키 y2 를 계산한다.  
4. B 는 $$K_s=y1^x2\;mod\;p$$ 로 비밀키 $$K_s$$ 를 생성한다.  
5. A 도 마찬가지로 $$K_s=y2^x1\;mod\;p$$ 로 $$K_s$$ 를 생성한다.  
