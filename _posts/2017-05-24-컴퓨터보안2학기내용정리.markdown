...
# Using Symmetric Encryption - pass  
...
Mutual authentication : 프토코ᅟᅩᆯ 보면 안다.
...
Ticket_{tgs} = E_{K_tgs}[K_{c, tgs} || ID_C || AD_C || ID_{tgs} || TS_2 || Lifetime_2]
$$

$$K_{c, tgs}$$ : k 하고 tgs 사이의 비밀키
...
Ticket_{tgs} 한테 주는 정보 $$E_{K_{tgs}}[K_{c, tgs}~||~ID_C~||~AD_C~||~ID_{Tgs}~||~...]$$
Ticket_v 한테 주는 정보  
